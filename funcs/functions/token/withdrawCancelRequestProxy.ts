...
module.exports = functions.runWith(runtimeOptions).https.onCall(async (data: RequestData, context) => {
  ...
  const res = await cfClient.withdrawalCancel({ user, ticket_id }).catch((err: Error) => err)
  if (res instanceof Error) {
    throw new functions.https.HttpsError('internal', 'failed withdrawCancel');
  }
});
